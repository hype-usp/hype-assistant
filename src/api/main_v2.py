...
with open(path_prompt_folder + 'augmented_response.txt', 'r') as file:
    
    prompt_augmented_response = file.read() 
#####################################################################
# functions
#####################################################################
...
def get_response(question, conversation_context, use_rl = False ,route_layer = rl):
    ...
    if use_rl and rl(question).name == 'bate-papo':
        ...
        augmented_response = llm.complete(
            prompt_augmented_response
        )
